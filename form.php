...
if($_GET) {
  ...
  if(empty($vid)) {
    $error = "You did not add your VID. Please make sure you fill out all the items.";
  } else if (empty($dates)){
    $error = "You did not add dates. Please make sure you fill out all the items.";
  } else  {
    TA::add_form_data($dates, $remarks, $vid);
    echo "Data added successfully....redirecting"
    ?>
      <script>
        document.location = "http://staff.ivao.in/tmcadmin/traineravail/";
      </script>
    <?php
  }
}
...
<label for="field1"><span>VID <span class="required">*</span></span>
  <input type="text" class="input-field" name="vid" value="<?php if($vid) { echo $vid; } ?>" />
</label>
<label for="field1"><span>Dates <span class="required">*</span></span>
  <input type="text" class="input-field" name="dates" value="<?php if($_GET['dates']) { echo $dates; } ?>" />
</label>
